...
class SentenceSentimentRegressor(Model):
    ...
    def create_placeholders(self):
        ...
        self.reason = tf.placeholder(tf.float32, [None, self.args["n_classes"]],
                                        name="input_reason")
    ...
    def build_model(self, metadata_path=None, embedding_weights=None):
        ...
        with tf.name_scope("CNN_LSTM"):
            ...
            self.classification_score = fully_connected(self.lstm_out, self.args["n_classes"])
        ...
        with tf.name_scope("loss"):
            ...
            self.loss_categorical = losses.categorical_cross_entropy(self.reason, self.classification_score)
        ...
        with tf.name_scope("Graph_Accuracy"):
            self.correct_preds = tf.equal(tf.argmax(self.classification_score, 1),
                                          tf.argmax(self.reason, 1))
            self.accuracy = tf.reduce_mean(
                                tf.cast(self.correct_preds, tf.float32),
                                name="accuracy")

    def train_step(self, sess, text_batch, sent_batch,reason_batch,
            ...
            feed_dict = {
                ...
                self.reason:reason_batch
            }
            ops = [self.tr_op_set, self.global_step, self.loss, self.out,self.loss_categorical,self.accuracy]
            if hasattr(self, 'train_summary_op'):
                ...
                _, step, loss, sentiment, loss_categorical, accuracy = sess.run(ops, feed_dict)
            # ask why?? they are implemented in the model???
            pco = pearsonr(sentiment, sent_batch)
            mse = mean_squared_error(sent_batch, sentiment)

            if verbose:
                time_str = datetime.datetime.now().isoformat()
                print("Epoch: {}\tTRAIN Regression{}: Current Step: {}\tLoss: {:g}\t"
                      "PCO: {}\tMSE: {}".format(epochs_completed,
                        time_str, step, loss, pco, mse))

                print(("Epoch: {}\tTRAIN Classification: {}\tCurrent Step: {}\tLoss {}\t"
                       "Accuracy: {}").format(epochs_completed,
                                              time_str, step, loss_categorical, accuracy))

            return pco, mse, loss, step, loss_categorical, accuracy

    def evaluate_step(self, sess, text_batch, sent_batch, reason_batch, verbose=True):
        """
