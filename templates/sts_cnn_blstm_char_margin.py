...
from models import CNNLSTMMargin

# Model Parameters
tf.flags.DEFINE_integer("embedding_dim", 100, "Dimensionality of character "
...
tf.flags.DEFINE_float("l2_reg_beta", 0.0, "L2 regularizaion lambda ("
                                            "default: 0.0)")
tf.flags.DEFINE_float("margin", 0.05, "Margin for the margin loss. Default: "
                                      "0.05")
tf.flags.DEFINE_float("C", 0.05, "Margin for the margin loss. Default: "
                                      "0.05")
tf.flags.DEFINE_integer("hidden_units", 128, "Number of hidden units of the "
                                             "RNN Cell")
tf.flags.DEFINE_integer("n_filters", 300, "Number of filters ")
tf.flags.DEFINE_integer("rnn_layers", 1, "Number of layers in the RNN")
...
tf.flags.DEFINE_boolean("bidirectional", True, "Flag to have Bidirectional "
                                               "LSTMs")
tf.flags.DEFINE_integer("sequence_length", 200, "maximum length of a sequence")
...
tf.flags.DEFINE_string("data_dir", "/scratch", "path to the root of the data "
                                           "directory")
tf.flags.DEFINE_string("experiment_name", "STS_CNN_BLSTM_MARGIN_CHAR",
                       "Name of your model")
...
def initialize_tf_graph(metadata_path, vocab_size):
    ...
    with sess.as_default():
        siamese_model = CNNLSTMMargin(FLAGS.__flags)
...
def train(dataset, metadata_path, w2v):
    ...
    with tf.Graph().as_default():
        ...
        while dataset.train.epochs_completed < FLAGS.num_epochs:
            train_batch = dataset.train.next_batch(negative_sampling=True,
                           batch_size=FLAGS.batch_size, tokenizer='nltk',
                           pad=siamese_model.args["sequence_length"])
            loss, step, good_sim, bad_sim = siamese_model.train_step(sess,
                                                 train_batch.s1,
                                                 train_batch.s2,
                                                 train_batch.s3,
                                                 train_batch.sim,
...
def evaluate(sess, dataset, model, step, max_dev_itr=100, verbose=True,
    ...
    while (dev_itr < max_dev_itr and max_dev_itr != 0) \
        ...
        val_batch = dataset.next_batch(negative_sampling=True,
                               batch_size=FLAGS.batch_size, tokenizer='nltk',
                                   pad=model.args["sequence_length"])
        val_loss, val_good_sim, bad_sim = \
            model.evaluate_step(sess, val_batch.s1, val_batch.s2,
                                val_batch.s3, val_batch.sim)
        avg_val_loss += val_loss

        all_dev_x1 += id2seq(val_batch.s1, dataset.vocab_i2w)
        all_dev_x2 += id2seq(val_batch.s2, dataset.vocab_i2w)
        all_dev_sims += val_good_sim.tolist()
...
if __name__ == '__main__':
    ...
    if FLAGS.dataset == 'STS':
        ...
        ds = STS(mode='semhash', load_w2v=False, lang='de', threshold=0.5,
              subset='GermanSTS')
    ...
    if FLAGS.mode == 'train':
        train(ds, ds.metadata_path, ds.vocab_size)
    elif FLAGS.mode == 'test':
        test(ds, ds.metadata_path, ds.vocab_size)
    elif FLAGS.mode == 'results':
        results(ds, ds.metadata_path, ds.vocab_size)
